...
**⚠️ 近期待办（非阻塞）**
1. 聚类 backend 接通（HDBSCAN crate 已就位，仅需 API route + 重建调度）
2. 设置 Tab 补 web_search / embedding / llm 配置 UI
3. 长中文 chunk 吞吐优化（2.6 → 10+ chunks/s）
4. OCR 支持（tesseract / PaddleOCR pipeline）
5. Golden set 多答案匹配（Q5 违约金 label 改进）
