...
- LocalSchedulerAppliance — 我们家的 RISC-V 一体机, 内置推理服务
- Unknown — fallback

不同形态的 LLM 默认策略不一样:
- Laptop / Server: 本地 LLM 安装维护成本高 (模型选型 / GPU 配置 / ROCm bug),
  默认走云端 token 更友好 (用户付月费即用)
- LocalSchedulerAppliance: 出厂预装 Ollama + 模型 (镜像构建时打入), 默认走本地
...
match form_factor {
    ...
    LocalSchedulerAppliance => 本地 Ollama auto-detect
}
```

用户可在 settings.llm.endpoint 手动覆盖. LocalSchedulerAppliance 检测条件: RISC-V arch
...
- 本地调度器设备出厂即可对话 (符合"零配置"承诺)
- LLM 配置成本透明: cloud token 用户买 quota, 本地调度器用户买硬件

**代价**:
- Laptop 用户必须配 cloud token 才能 chat (没有 demo mode)
- detect 错时 fallback: local scheduler 误判 Laptop 走 cloud (浪费 token); Laptop 误判
  local scheduler 试 Ollama (报 503). 用户可手 override
