...
/// Company mirror feed. It must serve the same signed `latest.json`; the updater
/// still verifies the minisign signature from `tauri.conf.json`.
pub const COMPANY_MIRROR_FEED: &str = "https://dl.engi-stack.com/attune/latest.json";
...
/// Resolve endpoints for a user-facing update-source preference.
///
/// `ATTUNE_UPDATE_FEED_URL` remains an operator override and wins when set.
/// Otherwise, `mirror` tries the company mirror first and GitHub second; any
/// other value uses GitHub only.
pub fn resolve_endpoints_for_source(source: Option<&str>) -> Vec<String> {
    let from_env = std::env::var(FEED_ENV_VAR).ok();
    if from_env.as_deref().map(str::trim).is_some_and(|s| !s.is_empty()) {
        return resolve_endpoints(from_env.as_deref());
    }
    match source.map(str::trim).map(str::to_ascii_lowercase).as_deref() {
        Some("mirror") => resolve_endpoints(Some(COMPANY_MIRROR_FEED)),
        _ => resolve_endpoints(None),
    }
}
...
mod tests {
    ...
    fn company_mirror_is_tried_before_github() {
        let mirror = COMPANY_MIRROR_FEED;
    ...
    #[test]
    fn source_preference_mirror_uses_company_mirror() {
        let eps = resolve_endpoints_for_source(Some("mirror"));
        assert_eq!(eps, vec![COMPANY_MIRROR_FEED, DEFAULT_GITHUB_FEED]);
    }
