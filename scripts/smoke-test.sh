...
#                    7. ATTUNE_FORM_FACTOR=local_scheduler env var override 生效
...
# ── 启动 server (no-auth 模式 + 模拟 local_scheduler 形态以测 form_factor) ────
# F-09-FORMFACTOR: 通过 ATTUNE_FORM_FACTOR=local_scheduler 测试 env var override 路径
...
ok "启动 attune-server-headless on $BASE_URL (HOME=$SMOKE_TMP, ATTUNE_FORM_FACTOR=local_scheduler)"
...
# ── 测试 8: F-09-FORMFACTOR — diagnostics 暴露 form_factor=local_scheduler ────
...
echo "$DIAG" | grep -q '"prefers_local_llm":true' || fail "Test 8/10: local_scheduler 形态应 prefers_local_llm=true"
ok "Test 8/10: F-09-FORMFACTOR /diagnostics 报 form_factor=local_scheduler + prefers_local_llm=true"

# ── 测试 9: F-09-FORMFACTOR — settings 默认 LLM = local scheduler endpoint ────
SETTINGS=$(curl -fsS -H "Authorization: Bearer $TOKEN" "$BASE_URL/api/v1/settings")
echo "$SETTINGS" | grep -q '"provider":"openai_compat"' || fail "Test 9/10: local_scheduler 形态应默认 llm.provider=openai_compat: $(echo "$SETTINGS" | head -c 240)"
echo "$SETTINGS" | grep -q '"endpoint":"http://127.0.0.1:8090/v1"' || fail "Test 9/10: local_scheduler 形态应默认走 local-scheduler endpoint: $(echo "$SETTINGS" | head -c 240)"
ok "Test 9/10: F-09-FORMFACTOR local_scheduler settings 默认 local-scheduler OpenAI-compatible endpoint"
