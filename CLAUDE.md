...
## 产品协同（Attune × lawcontrol）

**定位互补，平台共用，数据边界清晰**。详见 [`docs/product-collaboration-plan.md`](docs/product-collaboration-plan.md)。

- **lawcontrol**（律所级 B2B，`/data/company/lawcontrol`）— 多人协作、43+ 法律 skill、案件卷宗、合规审计
- **Attune**（个人级 B2C，本仓库）— 个人加密知识库、批注、AI 对话、本地优先

### 共用云管平台（关键约束）

**新增基础设施默认两产品共用，不做产品专属**：

- **PluginHub**：直接复用 lawcontrol 现有 `pluginhub/`（FastAPI + PG + License heartbeat），扩 `product_line` 字段分流。Attune 不建平行系统。
- **官网**：统一 monorepo — `attune.example.com` / `law.example.com` / `hub.example.com` / `docs.example.com` / `account.example.com`（SSO）/ `status.example.com`
- **SSO**：Keycloak 自托管，一个账号打通两产品
- **监控 / 支付 / 邮件 / CDN / 对象存储**：同一套（Stripe+支付宝、Sentry、Gatus 等）

### 商业协同

- 律所买 lawcontrol → 每席免费送 Attune Pro（捆绑）
- 个人律师买 Attune Pro → 所在律所升级 lawcontrol 时账号无缝迁移
- 硬件 + 3 年会员：Mini PC 预装两者，买断型销售

### 数据流硬边界

- ❌ Attune 个人 Chat / 批注 / 思考痕迹 → 永不同步到 lawcontrol
- ❌ lawcontrol 客户 PII → 绝不进 Attune（合规 / PIPL）
- ✅ 可互通（用户主动）：Attune 读书笔记 → lawcontrol 案件参考；lawcontrol 个人资料夹 → Attune 只读

### 技术互学（短期 action items）

- **Attune ← lawcontrol**：plugin.yaml+prompt.md+JSON schema 分离、Intent Router、律师插件 schema 对齐 `contract_review.output.schema`
- **lawcontrol ← Attune**：字段级加密（Argon2+AES-GCM）、上下文压缩 + Token Chip、批注加权 RAG

**插件 schema 跨产品一致**：两边共享 `@attune-lawcontrol/schemas` 包（未来独立 repo），确保用户数据跨产品无迁移成本。
...
启动时检测 RAM/GPU/NPU，推荐本地摘要模型。Settings 里展示"根据你的硬件推荐，可更换"，用户可覆盖但不改默认逻辑：

| RAM | GPU/NPU | 默认摘要模型 | 吞吐估计 |
|-----|---------|-------------|---------|
| ≥32 GB | 独显/NPU | `qwen2.5:7b` | ~1 s/chunk |
| 16-32 GB | 核显/NPU | `qwen2.5:3b` | ~2 s/chunk |
| 8-16 GB | 核显 | `qwen2.5:1.5b` | ~3 s/chunk |
| <8 GB | 仅 CPU | `llama3.2:1b` | ~5 s/chunk |
...
attune 必须在以下平台 + 硬件组合上可编译、可运行、测试通过：
...
**核心原则**: attune **本身不直接使用 GPU/NPU**。所有 AI 推理通过 HTTP 调用外部 Ollama 服务完成。因此：

- **NVIDIA GPU**: 用户安装 NVIDIA 驱动 + CUDA → 安装 Ollama → Ollama 自动检测并使用 GPU → attune 通过 HTTP 调用，零 GPU 代码
