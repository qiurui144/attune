...
| **attune-enterprise** | 律所 B2B 小团队 | Django + Vue + 19 容器 SaaS |
...
# (1) 硬编码中文 UI 字面量（toast / 属性值 / JSX 文本节点）— 排除 JSDoc/comment 行 (`*` / `//` 开头)
grep -rnP "(toast\([^)]*'[^']*[\x{4e00}-\x{9fff}]|(title|placeholder|label|description|aria-label)=\"[^\"]*[\x{4e00}-\x{9fff}]|>[^<{]*[\x{4e00}-\x{9fff}])" --include="*.tsx" . | grep -v "/i18n/" | grep -vE "^[^:]+:[0-9]+:\s*(\*|//)"
# (2) zh / en key 集合必须一致
...
MarketplaceView / SettingsView / Step3LLM / Step4Hardware 等）。**2026-05-25 D1 sweep 完成 0
