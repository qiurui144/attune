...
## 独立应用边界（与 lawcontrol 的关系）

Attune 是**独立应用**，不依赖任何外部系统在线。硬约束：

- **不调用 lawcontrol 的任何 API / pluginhub / 服务**。attune 必须能在没有 lawcontrol 部署的环境中完整工作。
- **不复用 lawcontrol 代码**（lawcontrol 是 Python + Django，attune 是 Rust，技术栈不同；即使能跨语言桥接，也优先自研以保持独立分发与单一二进制心智）。
- **可参考 lawcontrol 设计模式**：plugin.yaml + prompt.md + JSON schema 分离、Intent Router、chat_trigger 路由、case 卷宗心智、RPA 七类分法（rpa / crawler / search / skill / workflow / channel / industry，AI 边界严守 — 数据层禁用 AI、AI 层走 skill/workflow）。**实现完全独立**。
- **数据完全隔离**：attune 的 vault / 批注 / chat / Project / 知识库永远在用户本地（或用户自己的 K3），不与任何外部产品同步。
...
- **独立应用、不依赖 lawcontrol**（2026-04-25）：attune 不调用 lawcontrol 任何 API、不复用其代码。可参考其 plugin / RPA / Intent Router 设计模式，但实现完全独立。详见上文「独立应用边界」。
