...
DEFAULT_SCHEDULER_OCR_ENABLED=0
[ "$WITH_LONGTEXT" = "1" ] && DEFAULT_SCHEDULER_OCR_ENABLED=1
...
if [ -n "$LOCAL_SCHEDULER" ]; then
  SERVER_ENV+=(
    ...
    "ATTUNE_SCHEDULER_OCR_ENABLED=${ATTUNE_SCHEDULER_OCR_ENABLED:-$DEFAULT_SCHEDULER_OCR_ENABLED}"
    "ATTUNE_EMBED_QUEUE_BATCH_SIZE=${ATTUNE_EMBED_QUEUE_BATCH_SIZE:-${ATTUNE_SCHEDULER_EMBED_QUEUE_BATCH_SIZE:-512}}"
    "ATTUNE_SCHEDULER_EMBED_TASK_BATCH_SIZE=${ATTUNE_SCHEDULER_EMBED_TASK_BATCH_SIZE:-${ATTUNE_LOCAL_SCHEDULER_EMBED_TASK_BATCH_SIZE:-512}}"
    ...
    "ATTUNE_SCHEDULER_ASK_MAX_OUTPUT_TOKENS=${ATTUNE_SCHEDULER_ASK_MAX_OUTPUT_TOKENS:-${ATTUNE_LOCAL_SCHEDULER_ASK_MAX_OUTPUT_TOKENS:-${ATTUNE_LOCAL_ASK_MAX_OUTPUT_TOKENS:-24}}}"
...
if [ "$WITH_LONGTEXT" = "1" ]; then
  ...
  if [ -n "$LOCAL_SCHEDULER" ] && [ "$SCHEDULER_STRICT" != "0" ]; then
    ...
    export ATTUNE_LONGTEXT_REQUIRE_ANSWER_BUDGET_METADATA="${ATTUNE_LONGTEXT_REQUIRE_ANSWER_BUDGET_METADATA:-1}"
    export ATTUNE_LONGTEXT_SCHEDULER_GENERATION_P95_MS_MAX="${ATTUNE_LONGTEXT_SCHEDULER_GENERATION_P95_MS_MAX:-10000}"
    echo "      scheduler strict gate: generation=required prompt-cache=required answer-budget=required p95<=${ATTUNE_LONGTEXT_SCHEDULER_GENERATION_P95_MS_MAX}ms"
  fi
fi
RUN_STANDARD_CHAT=0
