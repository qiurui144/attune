...
**私有 AI 知识伙伴** — 本地决定，全网增强，越用越懂你的兴趣领域。

attune 是面向**任何领域个人知识工作者**的本地 AI 知识库（OSS Apache-2.0，零行业绑定）。行业用户（律师 / 医生 / 专利代理 / 学者）通过加载 `attune-pro/<行业>-pro` 插件包获得垂直能力，详见 [`docs/oss-pro-strategy.md`](../docs/oss-pro-strategy.md)。

单一静态 Rust 二进制约 30 MB，含完整 Web UI、TLS 和加密搜索引擎。
