...
fn e2e_classify_flow() {
    ...
    let result1 = classifier
        .classify_one("Rust 加密笔记", "关于 AES-GCM 的研究")
        .unwrap();
...
fn e2e_reclassify_flow() {
    ...
    let id = vault
        .store()
        .insert_item(&dek, "Item", "content", None, "note", None, None)
        .unwrap();
    ...
    vault
        .store()
        .update_tags(&dek, &id, &serde_json::to_string(&result).unwrap())
        .unwrap();
    ...
    vault
        .store()
        .update_tags(&dek, &id, &serde_json::to_string(&result2).unwrap())
        .unwrap();
...
fn e2e_classify_lock_unlock_persistence() {
    ...
    let id = vault
        .store()
        .insert_item(&dek, "Persistent", "c", None, "note", None, None)
        .unwrap();
