...
fn embedding_boundary_audit() {
    ...
    match &r {
        Ok((v, _usage)) => {
            ...
            assert_eq!(
                v[0].len(),
                emb.dimensions(),
                "empty input vector must have correct dims"
            );
            assert!(
                v[0].iter().all(|&x| x == 0.0),
                "empty input must produce all-zero vector"
            );
        }
    ...
    eprintln!(
        "{} single char 'a': {:?}",
        if r.is_ok() { "✅" } else { "❌" },
        r.as_ref().map(|(v, _)| v[0].len()).unwrap_or(0)
    );
    ...
    let same_all = vecs
        .iter()
        .all(|v| v.iter().zip(&vecs[0]).all(|(a, b)| (a - b).abs() < 1e-5));
    eprintln!(
        "{} determinism (5 repeats same): {}",
        if same_all { "✅" } else { "❌" },
        if same_all { "identical" } else { "DRIFT" }
    );
...
fn reranker_boundary_audit() {
    ...
    let docs100: Vec<String> = (0..100)
        .map(|i| format!("doc {} about rust programming", i))
        .collect();
    ...
    let lat = t.elapsed().as_secs_f64();
    eprintln!(
        "{} 100 doc batch in {:.1}s: scores count = {:?}",
        if r.is_ok() { "✅" } else { "❌" },
        lat,
        r.as_ref().map(|s| s.len())
    );
}
