...
impl LlmProvider for SynthMock {
    fn chat(&self, system: &str, _u: &str) -> attune_core::error::Result<(String, TokenUsage)> {
        Ok((
            self.reply(system),
            TokenUsage::empty("scripted", "test-model"),
        ))
    }
...
fn run_ref(
    ...
    run_skill(
        ...
        &inputs,
        true,
        &ref_resolver(reference, sources),
        &llm,
        "test-model",
    )
    .expect("ref run ok")
...
fn synthesis_example1_target_tcm_docx_roundtrip_chinese() {
    ...
    assert!(
        xml.contains("靶点") && xml.contains("EGFR"),
        "docx must carry the synthesis content"
    );
    assert!(
        xml.contains("黄芪") || xml.contains("免疫"),
        "second-domain content present"
    );
    // grounding成立: the section traced back to sources → not flagged unverified.
    assert!(
        !res.partial,
        "grounded synthesis is not partial: warnings={:?}",
        res.warnings
    );
}
...
fn synthesis_example2_eval_framework_pdf_roundtrip_chinese() {
    ...
    assert!(
        text.contains("评测") || text.contains("维度"),
        "pdf CJK must not garble: got {text:?}"
    );
}
...
fn synthesis_judge_grounds_abstractive_section() {
    ...
    assert!(
        !res.partial,
        "judge-credited section is grounded: warnings={:?}",
        res.warnings
    );
}
...
fn synthesis_fabricated_section_marked_unverified() {
    ...
    let mock = SynthMock {
        ...
        reduce: json!({"sections":[{"heading":"无关","body":"该模型由谷歌在火星上发布。"}]})
            .to_string(),
        judge: json!({"supported": true, "span_id": "c1", "evidence_quote": "谷歌在火星发布"})
            .to_string(),
    };
    let res = run_synth(&sources, mock, ExportFormat::Md);
    assert!(res.partial, "fabricated section must flag partial");
    assert!(
        res.warnings.iter().any(|w| w.contains("未能回溯")),
        "unverified warning present: {:?}",
        res.warnings
    );
    let md = String::from_utf8(res.artifact_bytes).unwrap();
    assert!(
        md.contains("需核实"),
        "unverified section must be marked [需核实] in the doc: {md}"
    );
}
...
fn reference_example4_docx_roundtrip_skeleton_and_content() {
    ...
    assert!(
        xml.contains("4K") && xml.contains("USB"),
        "grounded source content present"
    );
    assert!(
        !res.partial,
        "grounded reference doc not partial: {:?}",
        res.warnings
    );
}
...
fn reference_example4_pdf_roundtrip_chinese() {
    ...
    let text = pdf_text(&res.artifact_bytes);
    assert!(
        text.contains("项目背景") || text.contains("技术方案"),
        "pdf CJK skeleton: {text:?}"
    );
}
...
fn reference_ungrounded_content_marked_unverified() {
    ...
    let fabricated =
        json!({"paragraphs":["与素材完全无关的臆造内容关于量子飞船和外星科技。"]}).to_string();
...
fn edge_synthesis_single_source_still_produces_doc() {
    let res = run_synth(
        ...
        SynthMock {
            ...
            reduce: json!({"sections":[{"heading":"综述","body":"单一来源也能综合成文。"}]})
                .to_string(),
            judge: json!({"supported": false, "span_id": "", "evidence_quote": ""}).to_string(),
        },
        ExportFormat::Docx,
...
fn error_reference_cost_not_confirmed() {
    ...
    let err = run_skill(
        ...
        &inputs,
        false,
        &ref_resolver(REFERENCE_BID, &[("d", "x")]),
        &llm,
        "m",
    )
    .unwrap_err();
...
fn adversarial_injection_source_refused_in_synthesis() {
    ...
    assert!(
        res.warnings.iter().any(|w| w.contains("综述生成失败")),
        "warned: {:?}",
        res.warnings
    );
}
...
fn adversarial_injection_source_aborts_reference() {
    ...
    assert!(res.partial);
    assert!(
        res.warnings.iter().any(|w| w.contains("参考式生成失败")),
        "warned: {:?}",
        res.warnings
    );
}
...
fn adversarial_fabricated_content_cannot_pass_as_verified() {
    ...
    let mock = SynthMock {
        ...
        reduce: json!({"sections":[{"heading":"臆造","body":"明年量子计算机取代所有经典架构。"}]})
            .to_string(),
        judge: json!({"supported": false, "span_id": "", "evidence_quote": ""}).to_string(),
    };
    let res = run_synth(&sources, mock, ExportFormat::Md);
    assert!(res.partial, "fabricated content flags partial");
    assert!(String::from_utf8(res.artifact_bytes)
        .unwrap()
        .contains("需核实"));
}
...
fn concurrent_doc_skill_runs_independent() {
        ...
        .map(|i| {
            thread::spawn(move || {
                ...
                let mock = SynthMock {
                    ...
                    reduce,
                    judge: json!({"supported": false, "span_id": "", "evidence_quote": ""})
                        .to_string(),
                };
                let src_text = format!("方案编号 {i} 的内容。");
                let res = run_synth(
                    &[("a", Box::leak(src_text.into_boxed_str()))],
                    mock,
                    ExportFormat::Docx,
                );
    ...
    for h in handles {
        ...
        assert!(
            xml.contains(&format!("方案编号 {i}")),
            "run {i} carries its own data"
        );
    }
}
...
fn i18n_synthesis_traditional_chinese_docx() {
    let res = run_synth(
        ...
        SynthMock {
            ...
            reduce: json!({"sections":[{"heading":"並行","body":"自注意力機制提升並行度。"}]})
                .to_string(),
            judge: json!({"supported": false, "span_id": "", "evidence_quote": ""}).to_string(),
        },
        ExportFormat::Docx,
...
fn i18n_reference_english_docx() {
    let res = run_ref(
        ...
        &json!({"paragraphs":["The device offers resolution 4K, power 12W, interface USB."]})
            .to_string(),
        ExportFormat::Docx,
    );
    let xml = docx_text(&res.artifact_bytes);
...
fn degrade_synthesis_llm_garbage_still_downloads() {
    ...
    assert_eq!(
        &res.artifact_bytes[..2],
        b"PK",
        "valid docx even when degraded"
    );
}
...
fn both_skills_registered_as_oss() {
    ...
    let synth = reg
        ...
        .expect("research-synthesis registered");
    assert_eq!(synth.source, "oss");
    let refg = reg
        .get("reference-generate")
        .expect("reference-generate registered");
