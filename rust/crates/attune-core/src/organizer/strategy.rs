...
use super::types::{ClusterView, ItemView, RoleAssignment, LabelSource};

#[derive(Debug, Clone)]
pub struct ClusterLabel { pub name: String, pub summary: String, pub confidence: f32, pub source: LabelSource }

pub struct LabelCtx<'a> { pub llm: Option<&'a dyn LlmProvider> }
...
impl OrganizationStrategy for GenericStrategy {
    fn domain(&self) -> &str { "general" }
    fn label_cluster(&self, cluster: &ClusterView, ctx: &LabelCtx) -> Result<ClusterLabel> {
        // 优先 LLM 命名;无 LLM → extractive(取首 item 标题前若干词)。
        if let Some(llm) = ctx.llm {
            let titles: Vec<&str> = cluster.items.iter().take(8).map(|i| i.title.as_str()).collect();
            ...
            if let Ok((raw, _)) = llm.chat("你是文件归类助手。", &user) {
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(raw.trim()) {
                    if let Some(name) = v.get("name").and_then(|x| x.as_str()) {
                        return Ok(ClusterLabel { name: name.into(),
                            summary: v.get("summary").and_then(|x| x.as_str()).unwrap_or("").into(),
                            confidence: 0.8, source: LabelSource::Llm });
                    }
                }
            }
        }
    ...
    fn assign_role(&self, _item: &ItemView, _label: &ClusterLabel) -> Option<RoleAssignment> { None }
    fn suggest_project_kind(&self, _label: &ClusterLabel) -> String { "collection".into() }
}
...
pub fn extractive_label(cluster: &ClusterView) -> ClusterLabel {
    ...
    for it in cluster.items {
        for w in it.title.split(|c: char| !c.is_alphanumeric()).filter(|w| w.chars().count() >= 2) {
            *freq.entry(w.to_lowercase()).or_default() += 1;
        }
    }
    let name = freq.into_iter().max_by_key(|(_, c)| *c).map(|(w, _)| w).unwrap_or_else(|| "未命名分组".into());
    ClusterLabel { name, summary: String::new(), confidence: 0.4, source: LabelSource::Extractive }
}
...
impl StrategyRegistry {
    pub fn new() -> Self { Self { map: HashMap::new(), default: Arc::new(GenericStrategy) } }
    pub fn register(&mut self, s: Arc<dyn OrganizationStrategy>) { self.map.insert(s.domain().to_string(), s); }
    /// 精确 domain 命中 → 否则 default(Generic)。
    pub fn resolve(&self, domain: Option<&str>) -> Arc<dyn OrganizationStrategy> {
        domain.and_then(|d| self.map.get(d)).cloned().unwrap_or_else(|| self.default.clone())
    }
}
...
mod tests {
    ...
    fn generic_strategy_no_role_collection_kind() {
        ...
        let label = ClusterLabel { name: "topic".into(), summary: "".into(), confidence: 0.8, source: super::super::types::LabelSource::Extractive };
    ...
    fn dummy_item() -> super::super::types::ItemView {
        super::super::types::ItemView { item_id: "i".into(), title: "t".into(), content_snippet: "".into(), dir: "".into(), embedding: None }
    }
}
