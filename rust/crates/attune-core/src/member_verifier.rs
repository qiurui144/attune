...
use crate::cloud_client::{current_plan_grants_paid, plan_grants_paid, CloudClient};
...
pub enum MemberVerifyError {
    ...
    LicenseNotOnAccount,
    /// The client-provided account id does not match the authenticated session.
    #[error("account-mismatch: the claimed account does not match the authenticated session")]
    AccountMismatch,
    /// The license exists but is revoked.
    #[error("license-revoked: the claimed license has been revoked")]
    LicenseRevoked,
    /// A valid license exists but its plan is not a paid Attune plan.
    #[error("license-not-paid: the claimed license plan does not grant paid membership")]
    LicenseNotPaid,
}
...
pub trait MemberVerifier: Send + Sync {
    ...
    /// Verify against an already-fenced session source when the caller owns a
    /// cross-process account transaction. Test verifiers and other custom
    /// implementations keep their normal behavior by default; the production
    /// cloud verifier overrides this to avoid reopening the locked session
    /// file and to bind proof to the exact cookie being committed.
    fn verify_paid_with_session(
        &self,
        account_id: &str,
        license_id: &str,
        _session: &dyn CloudSessionSource,
    ) -> Result<(), MemberVerifyError> {
        self.verify_paid(account_id, license_id)
    }
}
...
impl CloudSessionSource for DiskCloudSession {
    fn load(&self) -> Option<(String, String)> {
        let session = crate::cloud_session::load_cloud_session().ok().flatten()?;
        Some((session.cloud_url, session.session))
    }
}

/// A path-bound store can also be injected directly. The server captures one
/// at `AppState` construction so later process-environment changes (notably
/// parallel tests, and embedders hosting multiple instances) cannot redirect
/// an in-flight member transaction to another session file.
impl CloudSessionSource for crate::cloud_session::CloudSessionStore {
    fn load(&self) -> Option<(String, String)> {
        let session = crate::cloud_session::CloudSessionStore::load(self)
            .ok()
            .flatten()?;
        Some((session.cloud_url, session.session))
    }
...
impl CloudSessionSource for crate::cloud_session::CloudSessionTransaction {
    fn load(&self) -> Option<(String, String)> {
        let session = crate::cloud_session::CloudSessionTransaction::load(self)
            .ok()
            .flatten()?;
        Some((session.cloud_url, session.session))
    }
...
impl<S: CloudSessionSource> MemberVerifier for CloudMemberVerifier<S> {
    fn verify_paid(&self, account_id: &str, license_id: &str) -> Result<(), MemberVerifyError> {
        verify_paid_from_session(&self.session, account_id, license_id)
    }
    ...
    fn verify_paid_with_session(
        ...
        account_id: &str,
        license_id: &str,
        session: &dyn CloudSessionSource,
    ) -> Result<(), MemberVerifyError> {
        verify_paid_from_session(session, account_id, license_id)
    }
}

fn verify_paid_from_session<S: CloudSessionSource + ?Sized>(
    session_source: &S,
    account_id: &str,
    license_id: &str,
) -> Result<(), MemberVerifyError> {
    ...
    let (cloud_url, session) = session_source
        .load()
    ...
    if account_id.trim() != me.id.to_string() {
        return Err(MemberVerifyError::AccountMismatch);
    }
    if !current_plan_grants_paid(&me.plan, me.plan_expires.as_deref()) {
        return Err(MemberVerifyError::LicenseNotPaid);
    }
    let licenses = client
        .list_licenses()
        .map_err(|e| MemberVerifyError::Unavailable(e.to_string()))?;

    // Match by license_key or either numeric id shape.  New login/restore
    // state uses `License::canonical_id`, while accepting the row id here
    // preserves compatibility with older clients and persisted state.
...
mod tests {
    ...
    impl CloudSessionSource for StaticSession {
        fn load(&self) -> Option<(String, String)> {
            Some((self.url.clone(), "session=test".into()))
        }
    }

    fn spawn_json_server(responses: Vec<&'static str>) -> (String, std::thread::JoinHandle<()>) {
    ...
    #[test]
    fn authenticated_account_must_match_the_claim() {
        let (url, server) = spawn_json_server(vec![
            r#"{"id":7,"email":"owner@example.test","plan":"pro"}"#,
        ]);
        let verifier = CloudMemberVerifier::new(StaticSession { url });
        assert_eq!(
            verifier.verify_paid("8", "license-1").unwrap_err(),
            MemberVerifyError::AccountMismatch
        );
        server.join().unwrap();
    }

    #[test]
    fn non_paid_license_plan_fails_closed() {
        let (url, server) = spawn_json_server(vec![
            r#"{"id":7,"email":"owner@example.test","plan":"pro"}"#,
            r#"[{"id":1,"plan":"individual","license_key":"license-1"}]"#,
        ]);
        let verifier = CloudMemberVerifier::new(StaticSession { url });
        assert_eq!(
            verifier.verify_paid("7", "license-1").unwrap_err(),
            MemberVerifyError::LicenseNotPaid
        );
        server.join().unwrap();
    }
