...
const COMMON_SURNAMES: &[&str] = &[
    ...
    "龙", "史", "陶", "黎", "贺", "顾", "毛", "郝", "龚", "邵", "万", "覃", "武", "钱", "戴", "严",
    "莫", "孔", "向", "汤",
];
...
fn extract_persons(text: &str) -> Vec<String> {
    ...
    const STOP_CHARS: &[char] = &[
        '同', '在', '是', '到', '与', '和', '已', '不', '了', '为', '的', '上', '说', '又', '却',
        '把', '将', '从', '向', '让', '签', '订', '约', '后', '前', '中', '里', '内', '外', '于',
        '由', '被', '对', '比', '给', '使', '叫', '令', '请', '来', '去', '入', '出', '回', '至',
        '及', '或', '而', '但', '即', '便', '才', '就', '该', '此', '这', '那', '其', '某', '会',
        '能', '可', '应', '需', '要', '想', '觉', '感', '认',
    ];
...
fn extract_arabic_amounts(text: &str) -> Vec<Amount> {
    ...
    let re = Regex::new(r"(\d{1,3}(?:,\d{3})+(?:\.\d+)?|\d+(?:\.\d+)?)\s*(亿|万|千|百|元|角|分)?")
        .unwrap();
    let mut out = Vec::new();
...
fn extract_chinese_capital_amounts(text: &str) -> Vec<Amount> {
    ...
    let re = Regex::new(r"[壹贰叁肆伍陆柒捌玖拾佰仟万亿元角分整]{2,}").unwrap();
    let mut out = Vec::new();
...
mod tests {
    ...
    fn extract_orgs_basic() {
        ...
        assert!(e
            .organizations
            .iter()
            .any(|o| o.contains("阿里巴巴集团") || o.contains("集团")));
