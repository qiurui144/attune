...
async fn amd_e2e_health() {
    ...
    let r = client()
        .get(format!("{host}/api/v1/status/health"))
        .send()
        .await
        .expect("GET health");
...
async fn amd_e2e_settings_has_hiapi() {
    ...
    let llm = &body["llm"];
    assert_eq!(
        llm["endpoint"].as_str().unwrap_or(""),
        "https://hiapi.online/v1"
    );
...
async fn amd_e2e_bind_directory_idempotent_after_g1_fix() {
    ...
    assert!(
        skipped > 0,
        "re-bind should skip existing files, got: {body}"
    );
}
...
async fn amd_e2e_search_ownership() {
    ...
    assert!(
        !results.is_empty(),
        "expected ≥1 hit on rust-book corpus, got {}",
        results.len()
    );
...
async fn amd_e2e_ocr_profiles_have_7_builtin_backend() {
    ...
    assert!(
        builtin_count >= 7,
        "expected 7 builtin profiles, got {builtin_count}"
    );
}
