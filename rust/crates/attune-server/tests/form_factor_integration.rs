...
//! → `/api/v1/settings` (post-vault-setup) reflects local-scheduler LLM defaults.
//!
...
/// F-09-FORMFACTOR Integration — local scheduler env var → diagnostics returns form_factor=local_scheduler
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "slow E2E (~60s/test, full Argon2id setup); R19 nightly only — run with --include-ignored"]
#[allow(clippy::await_holding_lock)]
async fn local_scheduler_env_var_propagates_to_diagnostics() {
    let _guard = TEST_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
    let tmp = tempfile::TempDir::new().expect("tmp");
    let (base, handle) = spawn_isolated_server(Some("local_scheduler"), &tmp).await;
    ...
    assert_eq!(
        ...
        "ATTUNE_FORM_FACTOR=local_scheduler should produce form_factor=local_scheduler, got: {:?}",
        hardware["form_factor"]
    );
    assert_eq!(
        hardware["prefers_local_llm"].as_bool(),
        Some(true),
        "local scheduler form factor should prefer local LLM"
    );
...
async fn laptop_default_propagates_to_diagnostics() {
    ...
    // CI / dev machines normally aren't local scheduler appliances — DMI keyword detection should
    // produce "laptop" or possibly "unknown" if /sys/class/dmi/id/product_name absent.
    // Either way, prefers_local_llm must be false (only local scheduler appliances prefer local).
    assert!(
        ff == "laptop" || ff == "unknown",
        "non-local-scheduler system should report laptop or unknown form_factor, got: {ff}"
    );
    assert_eq!(
        hardware["prefers_local_llm"].as_bool(),
        Some(false),
        "non-local-scheduler form factor must NOT prefer local LLM"
    );
...
/// F-09-FORMFACTOR + F-01-VAULT Integration — settings.llm reflects local scheduler default after vault setup
///
...
/// (F-01 vault-guarded). Without form_factor wiring, local scheduler images ship with empty
/// LLM endpoint and a confused first-launch UX.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "slow E2E (~60s/test, full Argon2id setup); R19 nightly only — run with --include-ignored"]
#[allow(clippy::await_holding_lock)]
async fn local_scheduler_form_factor_drives_settings_llm_default() {
    let _guard = TEST_MUTEX.lock().unwrap_or_else(|e| e.into_inner());
    let tmp = tempfile::TempDir::new().expect("tmp");
    let (base, handle) = spawn_isolated_server(Some("local_scheduler"), &tmp).await;
    let client = reqwest::Client::new();
    ...
    let setup_resp = client
        .post(format!("{}/api/v1/vault/setup", base))
        .json(&serde_json::json!({
            "password": "P@ssw0rd-FormFactorTest-LocalScheduler"
        }))
        .send()
        .await
        .expect("vault setup");
    ...
    // local scheduler form_factor → openai_compat + endpoint preset, but no pinned chat model
    let llm = settings.get("llm").expect("llm key in settings");
    assert_eq!(
        llm["provider"].as_str(),
        Some("openai_compat"),
        "local scheduler form factor should default LLM provider to openai_compat, got: {:?}",
        llm["provider"]
    );
    assert_eq!(
        llm["endpoint"].as_str(),
        Some("http://127.0.0.1:8090/v1"),
        "local scheduler form factor should preset scheduler endpoint, got: {:?}",
        llm["endpoint"]
    );
    assert!(llm["model"].is_null(),
        "local scheduler form factor should leave chat model unset so runtime can choose a local model, got: {:?}",
        llm["model"]);
