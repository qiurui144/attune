...
pub async fn classify_one(
    ...
    Ok(Json(
        serde_json::json!({"status": "ok", "id": id, "tags": result}),
    ))
}
...
pub async fn drain(State(state): State<SharedState>) -> AppResult<Json<serde_json::Value>> {
